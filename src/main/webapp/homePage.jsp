...
<c:choose>
    ...
    border:2px solid black; border-radius: 50%;" type="submit" value="${userName}"
        ...
        <h1 style="text-align:center"> Введите информацию о задаче </h1>

        <form action="controllers.addItems" method="post">
            <table style="margin-left:auto;margin-right:auto;">
                <tr>
                    <td>задача:</td>
                    <td>
                        <input type="text" name="ItemLabel" placeholder="введите задачу чтобы добавить" size="30"/>
                    </td>
                </tr>
                <tr>
                    <td>дата:</td>
                ...
                <tr>
                    <td></td>
                    <td>
                        <input class="btn btn-dark" type="submit" name="addTheItem" value="добавить">
                    </td>
                </tr>
                <tr>
        ...
        <br>
        <h2>
            <form action="controllers.FriendsItems" method="post"><input class="btn btn-dark" type="submit"
                                                                         name="friends"
                                                                         value="Списки друзей"></form>
        </h2>
        <h2>Мои задачи: </h2>
        <hr>
        <%
            ...
            <c:forEach var="tempItem" items="${listItems}">
                <tr>
                    ...
                    <form action="controllers.deleteItems" method="post">
                        <td style="padding:3px 20px 3px 20px;"><input type="hidden" name="id"
                                                                      value="${tempItem.get(0).id}"/> <input
                                class="btn btn-danger btn-sm" type="submit" value="удалить"/></td>
                    </form>
                                    ...
                                    <td style="padding:3px 20px 3px 60px;">
                                        ...
                                        <c:choose>
                                            <c:when test="${tempsubItem.check==1}">
                                                <input type="checkbox" id="chk" name="chkSms"
                                                       value="${tempsubItem.check}" checked="checked"
                                                       onclick="form.submit();"/>
                                            </c:when>
                                            <c:otherwise>
                                                <input type="checkbox" id="chk" name="chkSms"
